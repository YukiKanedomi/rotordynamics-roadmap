...
export const AGENDA: AgendaItem[] = [
  // ───────────── TIER A: 実装先行・設計法が未整備 ─────────────
  {
    ...
    bench: {
      date: '2026-07',
      summary: [
        '渦動比（WFR）の定量実測公表は事実上 Texas A&M（San Andrés 系）のみ — WFR=0.5/0.33/0.21 の3状態と側圧による安定化を査読公表。学術照合層（TAMU・NASA GRC・韓国勢）の主要データは2010年前後で止まり気味。',
        '量産組（Garrett・Hanwha・Aerzen・金士顿・MiTi）は適用規模（A3）の公称は厚い（BMW iX5 H2量産・100万回起動・5,000台etc.）が、渦動データ・解析照合の公開はゼロ。量産の安定性根拠が外に出ていない乖離が最大の特徴。',
        'IHI＋東北大は410,000rpm級マイクロ機で固定周波数サブシンクロナスの観測と粘弾性フィルムによる50%超低減という希少データ（2008、本文精読は残）。',
        '中国勢は産業化速度（金士顿=2018量産・冬季五輪バス搭載）に対し英語圏での学術公開が顕著に遅い（CNKI未走査＝本調査の限界）。',
        'RQ-01の5年ゴール（オンセット予測誤差の公表ベンチデータ）は、量産側が持つ実機データと学術側の照合手法の間に落ちており未占拠。',
      ],
      rows: [
        { name: 'Texas A&M（San Andrés 系）', levels: [2, 2, 0, 2], note: 'WFR定量実測の世界的にほぼ唯一の公表元。50krpm域・側圧包絡線・モデル照合まで一貫' },
        { name: 'NASA GRC（Howard/DellaCorte 系）', levels: [1, 2, 0, 2], note: '性能マップ＋XLGFBコード照合（2011）。WFR数値の独立公表は未確認' },
        { name: 'IHI（＋東北大）', levels: [2, 2, 3, 2], note: '410krpm級で固定周波数サブシンクロナス実測・抑制（2008）。FC用ETC-M・100kW級を製品化発表' },
        { name: 'KIST（RoMin）・KAIST・漢陽大', levels: [1, 2, 0, 2], note: '制御・モデル化（A4）主眼の論文構成。WFR定量の前面公表は未確認。漢陽大はスラストFB予測-実測（2015）' },
        { name: 'MiTi（Mohawk）', levels: [0, 2, 2, 0], note: '700,000rpm超の運転実証（技報）。量産主張はプレスのみで規模非公開' },
        { name: 'Garrett Motion', levels: [0, 1, 3, 0], note: 'FC量産（BMW iX5 H2）・100万回起動・25,000h公称＋HVAC 25–1,750kWc 量産へ。渦動データ公開ゼロ' },
        { name: 'Hanwha・Aerzen・金士顿（量産組）', levels: [0, 1, 3, 0], note: '5,000台超（Hanwha）・8万時間（Aerzen）・五輪バス搭載（金士顿）を公称。安定性根拠の公開なし' },
        { name: 'Xdot（Graham/Barber-Nichols）', levels: [0, 0, 1, 1], note: 'bumpフォイル減衰の簡易モデル（2006）等。設計・受託ポジションで製品スペックなし' },
      ],
      refs: ['TAMU-GFB-SideFeed-2009', 'Garrett-OilFree-2026', 'Xu-IMechE-FoilRotor-2024'],
    },
  },
  {
    id: 'RQ-02',
    ...
    bench: {
      date: '2026-07',
      summary: [
        '業界の逆説: AMB機は出荷時にドロップ試験を行っているはずだが、条件（回転数・回数）を査読レベルで公表したメーカーは Calnetix がほぼ唯一（全速ドロップ40回超＋部分速域200回超、~30,000rpm級フライホイール）。Waukesha は「全補助軸受フルスピードランディング試験付き」と公称するが条件非公開。',
        '再浮上（recontrol）の実証公表は Bath大学 Keogh グループのみ（アクチュエータ式TDB、2012）。ISO 14839系への学術関与も Keogh（Part 4 コンビナー）に集中——規格と実務の橋が実質1グループに依存。',
        'TDB寿命の定量評価法（許容ドロップ回数）は公表レベルでは世界的に不在。供給側は Cerobear（n·dm 360万公称）・Schaeffler（ドロップ後再使用可）ともプレス止まりで、SKF のみ BEAST シミュレーション＋実験検証（GT2013）を公表。',
        '追加発見: TU Darmstadt（Rinderknecht グループ）が TDB 専用試験リグ（外惑星型TDB）で2025年まで継続発表——プレイヤー台帳への追加候補。',
        'RQ-02の5年ゴール（ISO 14839-5 を実務に落とす寿命定量＋再浮上運用基準）は未占拠。メーカー内部に眠る試験データの体系化が最短経路。',
      ],
      rows: [
        { name: 'Calnetix', levels: [3, 1, 1, 0], note: '全速40回超・部分200回超のドロップを査読公表（JEGTP 2007）。dropカウンタは製品機能、寿命定量法の論文は無し' },
        { name: 'University of Bath（Keogh）', levels: [2, 2, 2, 3], note: 'TDB接触力計測（MSSP 2019）・再浮上実証（2012）・ISO TC108/SC2/WG7 コンビナー（Part 4 主導）' },
        { name: 'SKF（S2M＋軸受部門）', levels: [2, 1, 0, 1], note: 'BEAST によるドロップシミュレーション＋実験検証（GT2013）。試験条件は非公開' },
        { name: 'Waukesha Magnetic Bearings', levels: [1, 0, 0, 0], note: '全補助軸受フルスピードランディング試験付きと公称（条件・データ非公開）。事業存続形態も要確認' },
        { name: 'TU Darmstadt（Rinderknecht）', levels: [2, 1, 0, 0], note: 'TDB専用リグ・外惑星型TDBの実験公表（Actuators 2022〜GT2025）。台帳追加候補' },
        { name: 'Cerobear・Schaeffler（TDB供給側）', levels: [1, 1, null, 0], note: 'n·dm 360万実証・ドロップ後再使用可はプレス公称のみ。査読証拠なし' },
        { name: 'Everllence（HOFIM/Mecos）', levels: [0, 0, 0, 0], note: 'subsea 10万時間運転実績（2022）はあるがドロップ試験・TDBの公表なし' },
        { name: 'Danfoss Turbocor・Sulzer HST', levels: [1, 0, 0, 0], note: '製品資料にTDB記載のみ。量産実績は膨大だが試験公表なし' },
        { name: 'KAIST・KIMM・ZHAW・BC New Energy', levels: [0, 0, 0, 0], note: 'TDB特化の公表を確認できず（KAISTは制御系が主。中国語文献未走査）' },
      ],
      refs: ['Calnetix-Drop-JEGTP-2007', 'ISO14839-5-2022', 'Khatri-Hawkins-ISMB16'],
    },
  },
  {
    id: 'RQ-03',
