...
export const BOUNDARIES: Boundary[] = [
  {
    ...
    detail:
      '転がり軸受の実用上限。グリース潤滑は下側、アンギュラ玉＋ジェット潤滑で上側。超えると油膜（ティルティングパッド）→ガス/フォイル→AMBへ移行が強制され、その移行の代償（減衰の得失）が台帳に載る。',
    caution: '代表値は業界目安 [設計判断]（要出典・一次照合待ち）',
    sources: [],
    calc: true,
  },
  {
    ...
    law: 'σ_θ ≈ ρU²　(薄肉リング近似)',
    detail:
      'σ_allow=1,000 MPa・ρ=7,850 kg/m³ なら理論上限 U≈358 m/s。安全率・積層/磁石保持で実用は大きく下がる。高出力密度化は「径を増やす」方向をここで封じられ、軸方向に伸びるか回転数を上げるしかなく、どちらも B3 を押す。',
    caution: '材料帯（積層/金属スリーブ/CFRP）は代表帯 [設計判断]（要出典）',
    sources: [],
    calc: true,
  },
  {
  ...
  },
  {
    ...
    law: 'H₂: σ_allow↓ → U_max↓ → 段数↑↑',
    detail:
      '水素環境は高強度鋼の許容応力を下げ（水素脆化）、インペラ周速上限を通常機より低く制限する。ヘッド∝U² のため周速低減は段数をさらに押し上げ、B4 の「多段・長スパン化→危険速度律速」を増幅する。sCO2 側は高温×高密度が軸受・シール材料と減衰予測を同時に攻める。',
    caution: '[推定]（周速代表値の一次照合を open-questions に登録済み）',
    sources: [],
    calc: false,
  },
  {
    ...
    law: 'W_foil ≈ C·D·L·(D·N)　／　p_AMB ≲ B²/2μ₀',
    detail:
      'フォイル軸受の負荷容量は速度に比例的で、低速で浮上できず起動停止のたびに固体接触——摩耗寿命（固体潤滑、実証10万回級）が適用限界を規定。AMB は比荷重が磁気飽和で頭打ち＋停電時タッチダウン軸受要求が付随。油フリー化は無条件のトレンドではなく、適用領域の拡大として読む。',
    sources: ['DellaCorte-NASA-2016', 'DellaCorte-2012-MSSP', 'ISO14839-5-2022'],
    calc: false,
  },
]
